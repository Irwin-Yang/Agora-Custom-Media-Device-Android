...
public class GLUtils {
    ...
    public static int loadTexture(final java.nio.Buffer pixelsData, final Size size, final int usedTexId) {
        int textures[] = new int[1];
        if (usedTexId == NO_TEXTURE) {
            GLES20.glGenTextures(1, textures, 0);
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, textures[0]);
            ...
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, GLES20.GL_RGBA, size.width, size.height,
                    0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixelsData);
        } else {
            GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, usedTexId);
            GLES20.glTexSubImage2D(GLES20.GL_TEXTURE_2D, 0, 0, 0, size.width,
                    size.height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixelsData);
            textures[0] = usedTexId;
        }
        return textures[0];
    }

    public static int loadTexture(final java.nio.Buffer pixelsData, int width, int height, final int usedTexId) {
        int textures[] = new int[1];
        ...
        if (usedTexId == NO_TEXTURE) {
            ...
            GLES20.glTexParameterf(target,
                    GLES20.GL_TEXTURE_WRAP_T, GLES20.GL_CLAMP_TO_EDGE);
            GLES20.glTexImage2D(target, 0, GLES20.GL_RGBA, width, height,
                    0, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixelsData);
        } else {
            GLES20.glBindTexture(target, usedTexId);
            GLES20.glTexSubImage2D(target, 0, 0, 0, width,
                    height, GLES20.GL_RGBA, GLES20.GL_UNSIGNED_BYTE, pixelsData);
            textures[0] = usedTexId;
        }
        return textures[0];
    }
